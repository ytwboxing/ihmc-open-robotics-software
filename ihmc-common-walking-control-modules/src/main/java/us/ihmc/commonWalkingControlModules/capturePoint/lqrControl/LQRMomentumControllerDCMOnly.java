 ...
 * <p> C = </p>
 */
public class LQRMomentumControllerDCMOnly
{
   ...
   final DMatrixRMaj A = new DMatrixRMaj(3, 3);
   final DMatrixRMaj B = new DMatrixRMaj(3, 3);
   private final DMatrixRMaj BTranspose = new DMatrixRMaj(3, 3);
   final DMatrixRMaj C = new DMatrixRMaj(3, 3);
   final DMatrixRMaj D = new DMatrixRMaj(3, 3);

   final DMatrixRMaj A2 = new DMatrixRMaj(3, 3);
   final DMatrixRMaj A2Inverse = new DMatrixRMaj(3, 3);
   final DMatrixRMaj B2 = new DMatrixRMaj(3, 3);
   ...
   final DMatrixRMaj N = new DMatrixRMaj(3, 3);
   final DMatrixRMaj NTranspose = new DMatrixRMaj(3, 3);

   final DMatrixRMaj NB = new DMatrixRMaj(3, 3);

   final DMatrixRMaj S1 = new DMatrixRMaj(3, 3);
   private final DMatrixRMaj s2 = new DMatrixRMaj(3, 1);
   ...
   final DMatrixRMaj ARiccati = new DMatrixRMaj(3, 3);

   private final DMatrixRMaj A2InverseB2 = new DMatrixRMaj(3, 3);
   private final DMatrixRMaj DQ = new DMatrixRMaj(3, 3);
   private final DMatrixRMaj R1InverseDQ = new DMatrixRMaj(3, 3);
   private final DMatrixRMaj R1InverseBTranspose = new DMatrixRMaj(3, 3);
   private final DMatrixRMaj coefficients = new DMatrixRMaj(3, 1);

   final DMatrixRMaj A2Exponential = new DMatrixRMaj(3, 3);
   final DMatrixRMaj timeScaledA2 = new DMatrixRMaj(3, 3);
   final DMatrixRMaj summedBetas = new DMatrixRMaj(3, 1);

   private final DMatrixRMaj finalVRPState = new DMatrixRMaj(3, 1);

   private final DMatrixRMaj relativeState = new DMatrixRMaj(3, 1);
   private final DMatrixRMaj relativeDesiredVRP = new DMatrixRMaj(3, 1);

   final RecyclingArrayList<DMatrixRMaj> alphas = new RecyclingArrayList<>(() -> new DMatrixRMaj(3, 1));
   ...
   private final MatrixExponentialCalculator a2ExponentialCalculator = new MatrixExponentialCalculator(3);
   ...
   public LQRMomentumControllerDCMOnly(DoubleProvider omega)
   {
      this(omega, null);
   }

   public LQRMomentumControllerDCMOnly(DoubleProvider omega, YoVariableRegistry parentRegistry)
   {
      this(omega.getValue(), parentRegistry);
   }
   
   public LQRMomentumControllerDCMOnly(double omega, YoVariableRegistry parentRegistry)
   {
   ...
   {
      //MatrixTools.setMatrixBlock(A, 0, 0, CommonOps_DDRM.identity(3, 3), 0, 0, 3, 3, 1.0);
      MatrixTools.setMatrixBlock(B, 0, 0, CommonOps_DDRM.identity(3, 3), 0, 0, 3, 3, 1.0);
      MatrixTools.setMatrixBlock(C, 0, 0, CommonOps_DDRM.identity(3, 3), 0, 0, 3, 3, 1.0);
      MatrixTools.setMatrixBlock(D, 0, 0, CommonOps_DDRM.identity(3, 3), 0, 0, 3, 3, -1.0 / omega);
    ...
    */
   public void computeControlInput(DMatrixRMaj currentCOMPosition, DMatrixRMaj currentCOMVelocity, double time)
   {
      if (shouldUpdateS1)
         computeS1();
      ...
      relativeState.set(currentCOMPosition);
      for (int i = 0; i < 3; i++)
      {
         relativeState.add(i, 0, currentCOMVelocity.get(i) / omega.getDoubleValue());
         relativeState.add(i, 0, -finalVRPState.get(i));
      }

      relativeCoMPosition.set(relativeState);
      relativeCoMVelocity.set(relativeState);
      ...
      //u right now is dcm_dot, we want ddx
      //ddx = omega*(dcm_dot - dx)
      CommonOps_DDRM.addEquals(u, -1, currentCOMVelocity);
      CommonOps_DDRM.scale(omega.getDoubleValue(), u);
   }
